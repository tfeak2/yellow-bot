module.exports = {
    ...
    execute(message, args, Discord) {
        ...
        if (args[1] == "add" && args.length == 4) {
            if (!message.member.hasPermission("MANAGE_WEBHOOKS")) return message.channel.send("Failed to add link, you dont have permission.");
            links[args[2]] = [`**${args[2]}:**`, args[3]];
            return message.channel.send(`Successfully added ${args[2]}!`);
        }
        if (args[1] == "remove" && args.length == 3) {
            if (!message.member.hasPermission("MANAGE_WEBHOOKS")) return message.channel.send("Failed to remove link, you dont have permission.");
            delete links[args[2]];
        ...
        embed.setDescription("Some useful websites you might need");
